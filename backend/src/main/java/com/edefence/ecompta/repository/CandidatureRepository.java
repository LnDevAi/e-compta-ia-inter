...
public interface CandidatureRepository extends JpaRepository<Candidature, UUID> {
    ...
    @Query("SELECT c FROM Candidature c WHERE c.poste.id = :pid AND c.entreprise.id = :eid ORDER BY c.createdAt DESC")
    List<Candidature> findByPoste(@Param("eid") UUID eid, @Param("pid") UUID pid);
    ...
    @Query("SELECT COUNT(c) FROM Candidature c WHERE c.poste.id = :pid AND c.entreprise.id = :eid")
    long countByPoste(@Param("eid") UUID eid, @Param("pid") UUID pid);
}
